...
class PdfShuffler:
    ...
    def getPassword(self):
        dialog = gtk.MessageDialog(
            ...
            gtk.BUTTONS_OK,
            None)
        dialog.set_markup('Please enter password <b>key</b>:')
        entry = gtk.Entry()
        entry.set_visibility(False)
        entry.connect("activate", responseToDialog, dialog, gtk.RESPONSE_OK)
        hbox = gtk.HBox()
        hbox.pack_start(gtk.Label("Key:"), False, 5, 5)
        hbox.pack_end(entry)
        dialog.format_secondary_markup("This will be used for <i>authentication</i> purposes")
